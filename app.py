...
def create_excel_template(feature_columns):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        template.to_excel(writer, index=False, sheet_name="Student Data")
        worksheet = writer.book["Student Data"]
        ...
        for column_cells in worksheet.columns:
            worksheet.column_dimensions[column_cells[0].column_letter].width = 26
        ...
        if "Gender" in feature_columns:
            ...
            gender_validation.promptTitle = "Select Gender"
            gender_validation.prompt = "Please select Female or Male."
        ...
        if "ParentalSupport" in feature_columns:
            ...
            support_validation.promptTitle = "Select Parental Support"
            support_validation.prompt = "Please select Low, Medium, or High."
...
<div class="panel">
    ...
    <p class="panel-text">Each record represents one student. The prediction model was trained using student identification, engagement, learning habits, prior academic performance, parental support, extracurricular participation, and gender. The final feature list used by this app is loaded directly from the saved trained model artifacts.</p>
    <div class="info-grid">
...
with tab_predict:
    st.markdown(
        """
<div class="how-it-works"><p>Choose whether to predict one student manually or upload a completed Excel/CSV file to predict multiple students.</p></div>
""",
        unsafe_allow_html=True,
    )
    ...
    prediction_mode = st.radio(
        ...
        ["👤 Single Student", "📁 Bulk File Upload"],
        horizontal=True,
        key="prediction_mode",
    )

    if prediction_mode == "👤 Single Student":
        ...
        if submitted:
            ...
            if missing:
                ...
                new_student = pd.DataFrame([input_data]).reindex(columns=selected_features)
...
""",
    ...
    elif prediction_mode == "📁 Bulk File Upload":
        st.markdown(
            """
<div class="panel">
...
""",
        ...
        if "bulk_prediction_results" in st.session_state:
            ...
            if st.button("🗑️ Clear Bulk Results", key="clear_bulk_results"):
                del st.session_state["bulk_prediction_results"]
                st.rerun()
